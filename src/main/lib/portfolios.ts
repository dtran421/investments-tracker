...
import { DBSchema, Asset, Transaction } from "../../renderer/types/types";
import { fetchStockInfo, fetchStockQuotes, getAssetData } from "./stocks";

const PORTFOLIOS_DB_FILENAME = "portfolios.json";

export const initializePortfolio = async (_event: IpcMainInvokeEvent, portfolioName: string) => {
...
export const fetchPortfolio = async (_event: IpcMainInvokeEvent, portfolioSlug: string) => {
  const userDataPath = app.getPath("userData");
...
export const updateStockAsset = async (
  ...
  if (portfolio) {
    ...
    if (!updateField) {
      ...
      portfolio.assets[index] = _.set(portfolio.assets[index], field, value);
    }
    await db.write();
...
export const deleteStockAsset = async (_event: IpcMainInvokeEvent, portfolioSlug: string, tickerSymbol: string) => {
  const userDataPath = app.getPath("userData");
...
export const updateTxn = async (_event: IpcMainInvokeEvent, portfolioSlug: string, txn: Transaction) => {
  const userDataPath = app.getPath("userData");
  ...
  if (portfolio) {
    ...
    if (txn) {
      ...
      if (updateAsset) {
        ...
        const stockQuote = await fetchStockInfo(tickerSymbol);
        const { [tickerSymbol]: price } = await fetchStockQuotes(_event, [tickerSymbol]);
        ...
        if (stockQuote) {
          ...
          if (asset) {
            const { quantity: currShares, costBasis: currCostBasis } = asset;

            const newQuantity = currShares + shares;
            const newCostBasis =
              (currShares * currCostBasis + (mode === "buy" ? 1 : -1) * shares * price) / newQuantity;
          ...
          cash.quantity += (mode === "buy" ? -1 : 1) * Number((shares * price).toFixed(6));

          portfolio.assets = [
            modifiedStockQuote,
            ..._.filter(assets, ({ symbol }) => symbol !== tickerSymbol && symbol !== "Cash"),
            cash,
          ];
        } else {
          throw new Error("couldn't add new asset");
