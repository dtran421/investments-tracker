...
import { Portfolio } from "../../renderer/types/types";
...
export const fetchStockQuotes = async (_event: IpcMainInvokeEvent, tickerSymbols: string[]) => {
  try {
    ...
    const quotes = Object.fromEntries(await alpaca.getLatestTrades(tickerSymbols));
...
export const getAssetData = async (_event: IpcMainInvokeEvent, portfolio: Portfolio) => {
  try {
    const { assets } = portfolio;
    const tickerSymbols = _.map(_.slice(assets, 0, assets.length - 1), ({ symbol }) => symbol);
    ...
    if (Object.keys(stockPrices).length || assets.length === 1) {
      let assetData = _.map(assets, (asset) => {
        ...
        const percentGain = costBasis ? (profitPerShare / costBasis) * 100 : 0;
        return {
          ...asset,
          price,
          marketValue,
      ...
      const totalPortfolioValue = _.sum(_.map(assetData, ({ marketValue }) => marketValue));
