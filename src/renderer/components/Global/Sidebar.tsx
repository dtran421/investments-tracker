...
const PageButton = ({ icon, link, isActive, isExpanded, portfolioName, goToPage }: PageButtonProps) => {
  return (
      ...
      >
        <div className="flex flex-col items-center space-y-3">
          ...
          <div className="flex flex-col items-start space-y-3">
            {portfolios.map(({ slug: portfolioSlug, name: portfolioName }) => (
              <PageButton
                key={portfolioSlug}
                icon={
                  <AiOutlineStock
                    size={20}
                    className={`${
                      activePage === portfolioSlug ? "text-white" : "text-neutral-300 group-hover:text-orange-500"
                    }`}
                  />
                }
        ...
        <div className="flex justify-end">
          ...
          >
            {isExpanded ? <AiOutlineLeft size={18} /> : <AiOutlineRight size={18} />}
          </button>
        </div>
      </div>
    </div>
